...
class _SignInScreenState extends State<SignInScreen> {
  ...
  final _viewModel = locator<LoginModel>();

  void navigateToRegistration(BuildContext context) =>
      context.router.pushNamed('signup');

  void navigateToError(BuildContext context, String message) => showDialog(
      context: context,
      builder: (_) => ErrorPanel(message: message));

  @override
  void initState() {
    _viewModel.addListener(() {
      if(_viewModel.state == FetchingState.error){
        navigateToError(context, _viewModel.message ?? 'Нет соединения');
      }
    });
    super.initState();
  }
